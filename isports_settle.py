...
def api_get(path, params=None):
    ...
    res = requests.get(BASE_URL + path, params=params, timeout=REQUEST_TIMEOUT)
...
def unique_dates_for_pending(history):
    return sorted({item.get("date") for item in history if isinstance(item, dict) and item.get("result") == "pending" and item.get("date")})
...
def main():
    ...
    if not isinstance(history, list):
        history = []
    pending = [item for item in history if isinstance(item, dict) and item.get("result") == "pending"]
    ...
    for item in history:
        ...
        if new_result != "pending":
            ...
            item["final_score"] = f"{match.get('homeScore')}:{match.get('awayScore')}"
            updated += 1
            debug(f"SETTLED {item.get('match')} | {item.get('bet')} | {item.get('final_score')} -> {new_result}")
    save_json(RESULTS_FILE, history)
