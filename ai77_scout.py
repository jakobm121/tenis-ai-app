...
def score_name_match(search_name, candidate_name):
    s = normalize_name(search_name)
    ...
    score = int((overlap / max(len(s_words), len(c_words))) * 70)
...
def get_team_id(team_name):
    ...
    if not FOOTBALL_API_KEY:
        print(f"NO FOOTBALL_API_KEY for {team_name}")
    ...
    try:
        ...
        print(f"TEAM ID ERROR for {team_name}: {e}")
        team_id_cache[team_name] = None
        return None
...
def get_team_goal_stats(team_name):
    ...
    try:
        ...
        if valid_games == 0:
            print(f"FALLBACK USED for {team_name} (no valid finished games): {fallback}")
            team_stats_cache[team_name] = fallback
            return fallback
