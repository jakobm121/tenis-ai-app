...
def get_fixture_prediction_data(fixture_id):
    ...
    try:
        ...
        result["goals_home"] = gh if gh is not None and gh >= 0 else None
        result["goals_away"] = ga if ga is not None and ga >= 0 else None
    ...
    for fixture in fixtures:
        try:
            ...
            for market in odds_markets["totals"]:
                odds = market["price"]
                implied = market.get("avg_implied", 1 / odds)
                point = market["point"]
...
def build_predictions():
    ...
    # fallback fill to 5 with best remaining H2H first, then goals
    if len(final) < 5:
        fallback_pool = sorted(h2h_candidates + goal_candidates, key=lambda x: x["confidence"], reverse=True)
